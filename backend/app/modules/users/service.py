...
async def create_user(db: AsyncSession, current_user: User, payload: UserCreate) -> User:
    ...
    if payload.role == "employee" and payload.assigned_client_ids:
        ...
        for cid in payload.assigned_client_ids:
            mapping = EmployeeClient(employee_id=user.id, client_id=cid)
            db.add(mapping)
        await db.flush()
    ...
    db.add(
        ActivityLog(
            ...
            action="user_created",
            details={"user_id": str(user.id), "name": user.name, "role": user.role},
        )
    )
    return user
...
async def update_user(db: AsyncSession, current_user: User, user: User, payload: UserUpdate) -> User:
    ...
    if payload.assigned_client_ids is not None and user.role == "employee":
        ...
        for cid in payload.assigned_client_ids:
            db.add(EmployeeClient(employee_id=user.id, client_id=cid))
...
async def safe_delete_user(db: AsyncSession, user_id: uuid.UUID, current_user: User) -> None:
    ...
    if res_count > 0 or app_count > 0:
        raise HTTPException(
            ...
            detail="This employee has historical activity (resumes/applications). Deactivate instead.",
        )
    ...
    db.add(
        ActivityLog(
            user_id=current_user.id,
            action="user_deleted",
            details={"user_id": str(user_id), "name": user.name},
        )
    )
    await db.flush()
...
async def get_employee_performance_list(
    ...
    query = select(User).where(User.role == "employee", User.is_active == True).order_by(User.name)  # noqa: E712
...
async def get_sub_admins(db: AsyncSession) -> list[SubAdminResponse]:
    """Get all sub-admins with their assigned client & employee counts and lists."""
    result = await db.execute(
        select(User).where(User.role == "sub_admin", User.is_active == True).order_by(User.name)  # noqa: E712
    )
...
async def update_sub_admin(
    db: AsyncSession, sub_admin_id: uuid.UUID, payload: SubAdminUpdate
) -> SubAdminResponse:
